...
def main():
    ...
    parser.add_argument('--l1_reg_weight', type=float, default=0.01)
    ...
    parser.add_argument('--sign_product_weight', type=float, default=0.5,help='Sign Product Entropy 辅助损失权重')
    parser.add_argument('--sign_direction_weight', type=float, default=0.5, help='Sign Direction 辅助损失权重')
    parser.add_argument('--sdgnn_layers', type=int, default=2)
    parser.add_argument('--sdgnn_heads', type=int, default=1)
    parser.add_argument('--sdgnn_dropout', type=float, default=0.3)
    parser.add_argument('--sdgnn_no_residual', action='store_true',help='禁用 SDGNN 残差连接')

    # 结构平衡扩边参数
    parser.add_argument('--balance_topk', type=int, default=6,help='结构平衡扩边时每个节点最多保留的新增边数')
    parser.add_argument('--balance_threshold', type=float, default=0.5,help='扩边得分阈值，低于该阈值的候选边会被忽略')
    parser.add_argument('--balance_weighting', type=str, default='score',choices=['sign', 'score', 'tanh'],help='扩边后新边的权重计算方式')
    ...
    parser.add_argument('--prune_threshold', type=float, default=0.5,help='Gumbel 筛边阶段的权重阈值')
